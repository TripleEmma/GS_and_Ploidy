...
# (discussion section, quadratic regression)
...
GS_regression <- data.frame(env_proxy = rep(NA, length(proxies)), 
                            sp_Num = NA, resid_normality = NA, complexed = NA,
...
for (n in seq(1, length(proxies))){
    ...
    complex_mod <- phylolm(log_GS ~ proxy_scaled + proxy_sq_scaled + PL, data = df_tmp2,
                           phy = target_tree, model = "BM", boot = 10000)
...
write_out_table <- function(trait, sp_type, sp_table){
    ...
    output <- paste0("justified/global_species/", analysis_type, sp_type, "_complex_global.txt")
