...
LUI_cond <- function(end){
    ...
    LUI_file <- "results/03_proxies/LUI/LUI_overall_years.txt"
...
LUI_df_community <- map(c(2008:2020), LUI_cond) %>% bind_rows()

# LUI interannual variation
LUI_sd_cond <- function(end){
    year_end <- paste0("Year", end)
    LUI_file2 <- "results/03_proxies/LUI/LUI_sds.txt"
...
LUI_sd_community <- map(c(2008:2020), LUI_sd_cond) %>% bind_rows()
...
output <- "results/03_proxies/all_proxies_sp.txt"
...
output <- "results/03_proxies/all_proxies_community.txt"
...
output <- "results/06_manuscript/TableS1_F_test.txt"
write.table(Welch_F_test, sep = "\t",
            col.names = TRUE, row.names = FALSE,
            file = output)

######### plots
new_label <- c("Grazing", "Mowing", "Fertilization", "LUI", "Patch_size", "Area_weighted_patch_size", "Patch_number", "Edge_density", "NN_distance", "Hanski_connectivity")
df_long2 <- df_long
df_long2$region <- "All"
all_df_long <- rbind(df_long, df_long2)
all_df_long2 <- all_df_long %>% 
    filter(!grepl("_sd", env_proxies)) %>% 
    filter(env_proxies != "C.shape.ind.Gd", env_proxies != "C.PX.100.Gd")
all_df_long2$env_proxies <- as.character(all_df_long2$env_proxies)
a <- setNames(new_label, unique(all_df_long2$env_proxies))
b <- all_df_long2 %>% mutate(new_label = a[all_df_long2$env_proxies])
b$new_label <- factor(b$new_label, levels = new_label)
b$region <- factor(b$region, levels = unique(b$region))

set.seed(0305)
violinplots <- ggplot(b, aes(region, value, color = region)) +
...
png("results/06_manuscript/FigureS4_env_proxies_violin_plots.png", 
    width = 36, height = 32, units = "cm", res = 600)
violinplots
dev.off()

box_plots <- ggplot(b, aes(region, value, color = region)) +
...
png("results/06_manuscript/FigureS4_env_proxies_box_plots.png", 
    width = 36, height = 32, units = "cm", res = 600)
box_plots
dev.off()

# 03_proxies_selected.R
