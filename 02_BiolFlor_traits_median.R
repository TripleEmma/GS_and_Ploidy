...
biolFlor_raw <- biolFlor_raw %>% tidyr::fill(V1) # the same as last command line
...
# but only one value for ploidy, or the other way around. 
...
################################################################################
# focus on BE species; There is should be an easier way to reconcile the names....
...
# (2). if several records are found in Germany, the median of these record to represent this species;
...
# (4). if multiple records indicate as most common type, calculate the median of these records;
# (5). if not (1) or (2) or (3) or (4), calculate the median of the records
...
## ------------------- 
median_Germany_ploidy <- multi_ploidy %>% 
    # filter(BEname %in% Germany_ploidy) %>% 
    filter(dummyGermany == 1) %>%  # only records (of each species) occur in Germany are kept
    group_by(BEname) %>% 
    summarise(medianPloidy = round(median(ploidyLevel), 2))

median_common_ploidy <- multi_ploidy %>% 
    filter(BEname %in% most_common_ploidy) %>%
    filter(dummyMostCommon == 1) %>%
    group_by(BEname) %>%
    summarise(medianPloidy = round(median(ploidyLevel), 2))

median_left_ploidy <- left_ploidy %>% 
    group_by(BEname) %>% 
    summarise(medianPloidy = round(median(ploidyLevel), 2))

median_ploidy_df <- rbind(median_Germany_ploidy, median_common_ploidy, median_left_ploidy)
median_ploidy_vector <- setNames(median_ploidy_df$medianPloidy, median_ploidy_df$BEname)

#------ fill in the ploidy_df 
for (i in seq(nrow(ploidy_df))){
    if (ploidy_df$BEname[i] %in% names(median_ploidy_vector)){
        ploidy_df$usedPloidy[i] <- median_ploidy_vector[[ploidy_df$BEname[i]]]
    }
...
# (2). if several records are found "occurring in Germany", use the median of these records to represent the species;
# (3). if several records are found and none of them indicate "occurring in Germany", see if it is "TRUE" in `BENNET_LEITCH_2001`; if there are several records indicate "TRUE" in this column, use the median of these records to represent the species;
# (4). if several records are found and none of them indicate "occurring in Germany" nor "TRUE" in `BENNET_LEITCH_2001`, use the median of all these records to represent the species; 

multi_GS <- GS_df %>% filter(multiGS == "Y")
length(unique(multi_GS$BEname)) # 48
nrow(duplicate_GS_names)

# case(1) and (2)
Germany_GS <- multi_GS %>% group_by(BEname) %>% 
    filter(grepl("chromosome number known and occurring in Germany", quality)) %>% 
    pull(BEname)
length(unique(Germany_GS)) # 45
...
## ------------------- 
median_Germany_GS <- multi_GS %>% 
    filter(grepl("chromosome number known and occurring in Germany", quality)) %>%
    group_by(BEname) %>% 
    summarise(medianGS = round(median(DNAcontent_pg_2C, na.rm = TRUE), 2))

median_BENNET_GS <- multi_GS %>% 
    filter(BEname %in% BENNET_GS) %>%
    filter(grepl("TRUE", BENNET_LEITCH_2001)) %>%
    group_by(BEname) %>% 
    summarise(medianGS = round(median(DNAcontent_pg_2C, na.rm = TRUE), 2))

median_left_GS <- GS_left %>%
    group_by(BEname) %>%
    summarise(medianGS = round(median(DNAcontent_pg_2C, na.rm = TRUE), 2))

median_GS_df <- rbind(median_Germany_GS, median_BENNET_GS, median_left_GS)
median_GS_vector <- setNames(median_GS_df$medianGS, median_GS_df$BEname)

##------ fill in the GS_df
for (i in seq(nrow(GS_df))){
    if (GS_df$BEname[i] %in% names(median_GS_vector)){
        GS_df$usedGS[i] <- median_GS_vector[[GS_df$BEname[i]]]
    }
...
output <- paste0(output_dir, "BE_species_BiolFlor_only_median.txt")
write.table(both_df,
            col.names = TRUE, row.names = FALSE, sep = "\t",
            file = output)
