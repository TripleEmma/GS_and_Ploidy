...
sp_mean_cover_each_plot <- merge(sp_mean_cover_each_plot0, sp_name_cols, by = "useful_name") # why this step? It should be fine to skip this step.
...
resident_sp_yearly_cover_each_plot <- as.data.frame(apply(resident_sp_cover_each_plot0, 2, 
                                                          function(x){x/sum(x, na.rm = TRUE)})) %>% 
    rownames_to_column("Year") %>% 
    pivot_longer(-Year, names_to = "ID", values_to = "Cover") %>% 
    separate(col = ID, into = c("useful_name", "PlotID"), sep = "__") %>% 
    arrange(PlotID) 
output <- paste0(output_dir, "resident_sp_yearly_relative_cover_each_plot_long.txt")
write.table(resident_sp_yearly_cover_each_plot,
            col.names = TRUE, row.names = FALSE, sep = "\t",
            file = output)
