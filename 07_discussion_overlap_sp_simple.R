...
write_out_table <- function(trait, sp_type, sp_table){
    ...
    output <- paste0("results/04_species_level/overlap_sp/", analysis_type, sp_type, "_overlap_sp_simple.txt")
...
# write_out_table <- function(trait, sp_type, sp_table){
#     analysis_type <- ifelse(trait == "GS", "GS_phylolm_", "PL_glm_")
#     output <- paste0("results/04_species_level/", sp_type, "/", analysis_type, sp_type, "overlap_conservative_simple.txt")
#     write.table(sp_table, sep = "\t",
#                 col.names = TRUE, row.names = FALSE,
#                 file = output)
# }
